...
\todo{
	\begin{itemize}
	\item What is the problem with alternative approaches eg. simple search predefined keywords, customer inquiry systems, ...
	\item discuss shortly machine learning and one vs two stage
	\end{itemize}	
}

Recent years have shown a trend for companies from buying standalone software with big service contracts products to buying small cloud products, also known as software as a service (SaaS).
...
As Dubey and Wagle said \todo{cite}, ``buyers have more flexibility to switch vendors and perhaps fewer headaches in maintaining the software".
Especially young companies are more likely to build SaaS, as it gives them the flexibility to adapt to the growth of the company in the future.
...
However, this introduces a new problem for the salesmen: It is harder to find potential customers.
...
As this training data does exist, it would need to be created manually by the salesmen.
...
A company may have twenty different brochures, but it never has one million.
To overcome this short corpus problem \todo{correct name?}, we employ feature selection to only select the most relevant words and ignore the noise.
...
Section~\ref{sec:concept} shows our two-staged classification and how we use feature selection to overcome the short corpus problem.
